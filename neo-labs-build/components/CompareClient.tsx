...
export function CompareClient({ labs, initialIds }: { labs: Lab[]; initialIds: string[] }) {
          ...
          <div style={{overflowX: 'auto'}}>
            <table>
              ...
              <tbody>
                ...
                <tr><td><strong>创始人</strong></td>{compareLabs.map(l => <td key={l.slug} style={{fontSize: 11}}>{l.founders.slice(0, 120)}...</td>)}</tr>
                <tr><td><strong>商业化阶段</strong></td>{compareLabs.map(l => <td key={l.slug}>S{l.commercialization_stage_score}</td>)}</tr>
                <tr><td><strong>团队稳定</strong></td>{compareLabs.map(l => <td key={l.slug}>S{l.team_stability_score}</td>)}</tr>
                <tr><td><strong>OD 校友</strong></td>{compareLabs.map(l => <td key={l.slug}>{l.founder_openai_deepmind_alumni_count}</td>)}</tr>
                <tr><td><strong>数据时效</strong></td>{compareLabs.map(l => <td key={l.slug}>{l.data_freshness_days}天</td>)}</tr>
                <tr><td><strong>末轮</strong></td>{compareLabs.map(l => <td key={l.slug}>{l.last_round?.type || 'N/A'} {l.last_round?.date || ''}</td>)}</tr>
                <tr><td><strong>下一轮</strong></td>{compareLabs.map(l => <td key={l.slug}>{l.next_expected_round?.type || 'N/A'} ({l.next_expected_round?.expected_window || 'N/A'})</td>)}</tr>
